...
describe('Transaction e2e test', () => {
  ...
  it('should create Transaction', async () => {
    ...
    await transactionUpdatePage.setNameInput(name);
    await transactionUpdatePage.setUuidInput(uuid);
    await transactionUpdatePage.setAmountInput(amount);
    await transactionUpdatePage.setPaymentDetailsInput(paymentDetails);
  ...
  it('should view the last Transaction', async () => {
    ...
    if (isVisible && (await transactionDetailPage.pageTitle.isDisplayed())) {
      ...
      expect(await transactionDetailPage.getNameInput()).toEqual(name);

      expect(await transactionDetailPage.getUuidInput()).toEqual(uuid);

      expect(await transactionDetailPage.getAmountInput()).toEqual(amount);

      expect(await transactionDetailPage.getPaymentDetailsInput()).toEqual(paymentDetails);
    }
  });
