...
export class InvoiceItemComponentsPage {
  ...
  noResult = element(by.cssContainingText('ion-label', 'No Invoice Items found.'));
  entities = element.all(by.css('page-invoice-item ion-item'));
...
export class InvoiceItemDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
